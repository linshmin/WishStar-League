...
function enterApp() {
  ...
  // Worklog form only for partners and leaders
  const wlForm = document.getElementById('worklog-form-section');
  if (wlForm) wlForm.classList.toggle('hidden', isAdmin);
  navigateTo('home');
  updatePendingBadge();
}
...
function openArticle(id) {
  ...
  const body = `<h3>${esc(a.title)}</h3>
    <div style="margin-bottom:1rem;">
      <span class="article-tag" data-cat="${a.category}">${a.category}</span>
      <span style="font-size:0.8rem;color:var(--text-muted);margin-left:0.5rem;">${esc(a.author)} · ${date}</span>
    </div>
    <div style="line-height:1.8;white-space:pre-wrap;font-size:0.92rem;">${esc(a.content)}</div>`;
  openModal(body);
}
...
function renderPublish() {
  ...
  // My articles
  const articles = loadData(STORAGE_KEYS.articles) || [];
  const mine = articles.filter(a => a.authorId === currentUser.id).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
  document.getElementById('my-articles').innerHTML = mine.length ?
    mine.map(a => {
      ...
      return `<div class="article-item" onclick="openArticle('${a.id}')">
        ...
        <div class="article-meta">${date} · 對象：${a.target === 'all' ? '所有人' : esc(a.target)}</div>
      </div>`;
